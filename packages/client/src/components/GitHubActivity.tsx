...
function buildGrid(contributions: Contribution[], weeksToShow: number) {
  ...
  while (current <= today) {
    ...
    currentWeek.push(
      lookup.get(dateStr) ?? { date: dateStr, count: 0, level: 0 },
    );
  ...
  const filteredLabels = monthLabels.filter((m, i) => {
    const next = monthLabels[i + 1];
    if (!next) return true;
    return next.index - m.index >= 3;
  });

  return { weeks, monthLabels: filteredLabels };
}
      ...
      <>
        <div className="flex items-baseline justify-between mb-2">
          <span className="text-xs text-muted-foreground">
            {total.toLocaleString()} contributions in the last {weeksToShow}{" "}
            weeks
          </span>
          <a
